...
// TO DO list:
// Boot directory: 	https://ksp-kos.github.io/KOS/general/volumes.html#boot - Doesnt work as expeted. Will not autorun a script.
// Node scripting: 	https://ksp-kos.github.io/KOS/tutorials/exenode.html
// Create GUI:			https://ksp-kos.github.io/KOS/structures/gui.html#gui
// Telnet server:		https://ksp-kos.github.io/KOS/general/telnet.html
// Use sounds:			https://ksp-kos.github.io/KOS/general/skid.html
...
// Reset inclination
if abs(launchlatitude) > abs(targetincl) {
	set targetincl to launchlatitude.
}

// Calculate Launch Azimuth
if targetincl >= 0 {
	set betaI to arcsin(cos(targetincl) / cos(launchlatitude)).
} else if targetincl < 0 {
	set betaI to 180 - arcsin(cos(targetincl) / cos(launchlatitude)).
}
set Vtargetorbit to sqrt(MUkerbin / (Rkerbin + targetaltitude / 1000)). //kilometers per second
if targetincl >= 0 {
	lock beta to arctan((Vtargetorbit * sin(betaI) - Vlaunchsite * cos(launchlatitude)) / (Vtargetorbit * abs(cos(betaI)))).
} else if targetincl < 0 {
	lock beta to 180 - arctan((Vtargetorbit * sin(betaI) - Vlaunchsite * cos(launchlatitude)) / (Vtargetorbit * abs(cos(betaI)))).
}
...
UNTIL runmode = 0 {
	...
	if should_stage {
	  stage.
		// Wait 5. is needed when using collapsable bell CE-10 and CE-60 cryogenic rocket engines
		wait 5.
	}
...
//		if SHIP:ALTITUDE > 70000 and ETA:APOAPSIS > 30 {
		...
		if VERTICALSPEED < 0 {
			set runmode to 6.
				...
				set warp to 0.
				set runmode to 6.
		}	else if circburnflag = 1 and ETA:APOAPSIS < 10 {
				RCS off.
				set warp to 0.
				set runmode to 6.
		}
...
FROM {
	local countdownrelease is 5.
	} UNTIL countdownrelease= 0 STEP {
		SET countdownrelease to countdownrelease - 1.
	} DO {
